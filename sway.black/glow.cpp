...
void Glow::Run()
{
    for (auto i = 0; i < g_GlowObjManager->m_GlowObjectDefinitions.Count(); i++) {
        ...
        case ClassId_CCSPlayer:
        {
            ...
            if (entity->HasC4() && is_enemy && options::glow_c4_carrier) {
                color = options::color_glow_c4_carrier;
                break;
            }

            if (!options::glow_players || !entity->IsAlive())
                continue;

            if (!is_enemy && options::glow_enemies)
                continue;

            color = is_enemy ? options::color_glow_enemies : options::color_glow_ally;

            break;
        }
        case ClassId_CChicken:
            if (!options::glow_chickens)
                continue;
            entity->m_bShouldGlow() = true;
            color = options::color_glow_chickens;
            break;
        case ClassId_CBaseAnimating:
            if (!options::glow_defuse_kit)
                continue;
            color = options::color_glow_defuse_kit;
            break;
        case ClassId_CPlantedC4:
            if (!options::glow_planted_c4)
                continue;
            color = options::color_glow_planted_c4;
            break;
        default:
        {
            if (entity->IsWeapon()) {
                if (!options::glow_weapons)
                    continue;
                color = options::color_glow_weapons;
            }
        }
        }
